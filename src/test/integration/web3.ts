...
describe('testing web3 functionality', () => {
  ...
  it('should successfully transfer eth from one account to another', async () => {
    ...
    expect(parseInt(await web3.eth.getBalance(coinbase), 10)).to.be.greaterThan(10000)
    // Asserting that new account is empty
    expect(parseInt(await web3.eth.getBalance(testAccount), 10)).to.eq(0)
    ...
    expect(parseInt(await web3.eth.getBalance(testAccount), 10)).to.eq(transferValue)
  })
